...
//  Cptimeouts.ReadIntervalTimeout         = MAXDWORD;	// MAXDWORD gives non-blocking read
  Cptimeouts.ReadIntervalTimeout         = 1;	// 1 gives blocking read
...
int SendByte(int comport_number, unsigned char byte)
{
  ...
  int ret = WriteFile(Cport[comport_number], &byte, 1, (LPDWORD)((void *)&n), NULL);
  printf( "%s: ret=%d\n", __func__, ret);
